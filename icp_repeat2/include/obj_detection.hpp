...
public:
	segmatation_sel()
	{
		...
		pcl_sub = nh.subscribe("/sunny_topic/tof_frame/pointcloud",1,&segmatation_sel::cloudCB, this);//定义接收者
		pcl_sub_livox = nh.subscribe("/livox/lidar",1,&segmatation_sel::cloudCB_livox, this);//定义接收者
		...
		tof_org_pub= nh.advertise<sensor_msgs::PointCloud2>("tof_org",1);//定义发布者
		livox_pub = nh.advertise<sensor_msgs::PointCloud2>("livox",1);//定义发布者
		livox_obj_pub = nh.advertise<sensor_msgs::PointCloud2>("livox_obj",1);//定义发布者
		...
		range_y=ros::param::param<double>("range_y",5);
		range_z=ros::param::param<double>("range_z",1.5);

		livox_range_x=ros::param::param<double>("livox_range_x",10);
		livox_range_y=ros::param::param<double>("livox_range_y",10);
		livox_range_z=ros::param::param<double>("livox_range_z",2);

		detec_y=ros::param::param<double>("detec_y",1);
		nearest_dis=ros::param::param<double>("nearest_dis",0.2);
		nan_ground_num=ros::param::param<int>("detnan_ground_numec_x",3);

		car_length=ros::param::param<double>("car_length",0.696);
		car_width=ros::param::param<double>("car_width",0.452);
		car_height=ros::param::param<double>("car_height",0.719);
		obj_wait_time=ros::param::param<double>("obj_wait_time",5.0);
		enable_planning=ros::param::param<int>("enable_planning",0);


		ros::param::get("~T_tof_to_livox",T_tof_to_livox_vel);
		T_tof_to_livox_M.setIdentity();
		T_tof_to_livox_M << T_tof_to_livox_vel[0], T_tof_to_livox_vel[1], T_tof_to_livox_vel[2],T_tof_to_livox_vel[3],
	     T_tof_to_livox_vel[4], T_tof_to_livox_vel[5], T_tof_to_livox_vel[6],T_tof_to_livox_vel[7],
	     T_tof_to_livox_vel[8], T_tof_to_livox_vel[9], T_tof_to_livox_vel[10],T_tof_to_livox_vel[11],
		T_tof_to_livox_vel[12], T_tof_to_livox_vel[13], T_tof_to_livox_vel[14],T_tof_to_livox_vel[15];
	...
	{
		//find max and min
		pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in_tof(new pcl::PointCloud<pcl::PointXYZ>());
		for(int i = 0; i < cloud_input->points.size(); i++)
		{
			...
			// if ((abs(cloud_input->points[i].y)<(range_y/2))&&((cloud_input->points[i].z>(tof_height-0.5))&&(cloud_input->points[i].z<(tof_height+range_z)))&&(cloud_input->points[i].x>0.01&&cloud_input->points[i].x<range_x))
			if ((abs(cloud_input->points[i].x)<(range_y/2))&&((cloud_input->points[i].y>(tof_height-0.5))&&(cloud_input->points[i].y<(tof_height+range_z)))&&(cloud_input->points[i].z>0.01&&cloud_input->points[i].z<range_x))
			{
				cloud_in_tof->points.push_back(cloud_input->points[i]);
			}
		}

    	pcl::transformPointCloud (*cloud_in_tof, tof_transformed_cloud, T_tof_to_livox_M);
		pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in(new pcl::PointCloud<pcl::PointXYZ>());
		*cloud_in=tof_transformed_cloud;
		...
		for (size_t i = 0; i < cloud_in->size(); i++)
		{
			...
			int64_t position=(int64_t)((p_v.x-x_min)/voxel_size)*Num_y+(int64_t)((p_v.y-y_min)/voxel_size);
			...
			if (iter != voxel_map.end()) 
			{
				...
				if(p_v.z<(voxel_map[position]->lowest_point.z))
				{
		...
		std::vector<bool> first_show(Num_y);
				...
				if (iter != voxel_map.end()) 
				{
					...
					if(first_show[j]==true)
					{
						...
						else
						{
							...
							{
								if (iter->second->contain_points[l].z<(p_low.z+point_to_groundsym))
								{
						...
						voxel_map[position_i]->is_deal=true;
					...
					else
					{
						...
						{
							int64_t position_last=(i-k)*Num_y+j;
							...
							if (iter_last != voxel_map.end())
							{
								...
								{
									Eigen::Vector2d last_point=Eigen::Vector2d(iter_last->second->lowest_point.x,iter_last->second->lowest_point.z);
									Eigen::Vector2d cur_point=Eigen::Vector2d(iter->second->lowest_point.x,iter->second->lowest_point.z);
									...
									if (voxel_map[position_i]->is_ground==1)
									{
										if (nanground_count<=nan_ground_num)
										{
											for (size_t m = 0; m< cloud_nanground_temp.size(); m++)
											{
												...
												{
													if (voxel_map[cloud_nanground_temp[m]]->contain_points[l].z<(voxel_map[cloud_nanground_temp[m]]->lowest_point.z+point_to_groundsym))
													{
												...
												voxel_map[cloud_nanground_temp[m]]->is_deal=true;
											}
										}
										else
										{
		...
		for (auto iter =voxel_map.begin(); iter!=voxel_map.end(); iter++)
		{
			if (!iter->second->is_deal)
			{
				cloud_nanground_final+=iter->second->contain_points;
				iter->second->nonground_points=iter->second->contain_points;
				iter->second->is_deal=true;
			}
		}	
	}
	...
	void obj_detection()
	{
		int64_t voxel_num_y=(int64_t)((detec_y/2)/voxel_size)+1;
		int64_t zero_y=(int64_t)((0-y_min)/voxel_size);
		int64_t dt_num_x=(int64_t)(range_x/voxel_size);
		if (zero_y<voxel_num_y)
		{
			voxel_num_y=zero_y;
		}	
		for (size_t i = zero_y-voxel_num_y; i < zero_y+voxel_num_y; i++)
		{
			for (size_t j = 0; j < dt_num_x; j++)
			{
				int64_t position_obj=j*Num_y+i;
		...
		if (cloud_obj_cloud.size()>=3)
		{
			...
			obj_distance=(sqrt(KDtree_dis[0])+sqrt(KDtree_dis[1])+sqrt(KDtree_dis[2]))/3;
		}
	...
	// void Pointcloud_to_2d_grid(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in, nav_msgs::OccupancyGrid& msg, double map_resolution)
	// {
	// msg.header.seq = 0;
	// msg.header.stamp = ros::Time::now();
	// msg.header.frame_id = "2d_map";
	...
	void cloudCB(const sensor_msgs::PointCloud2& input)
	{
		...
		std::cout<<"enable_planning:"<<enable_planning<<endl;
		if(enable_planning)
		{
			if (obj_symble==2)
			{
			...
			now_obj_time= ros::Time::now();

			if (face_obj)
			{
				std::cout<<"now_obj_time:"<<now_obj_time.toSec()<<std::endl;
				std::cout<<"last_obj_time:"<<last_obj_time.toSec()<<std::endl;
				std::cout<<"(now_obj_time-last_obj_time):"<<(now_obj_time-last_obj_time).toSec()<<std::endl;
				if ((now_obj_time-last_obj_time).toSec()>obj_wait_time)
				{
					planning_Num++;
		...
		pcl::toROSMsg(tof_transformed_cloud,output);
		output.header.frame_id = "livox";
		tof_org_pub.publish(output);
	...
	void cloudCB_livox(const sensor_msgs::PointCloud2& input)
	{
		pcl::PointCloud<pcl::PointXYZ> cloud;
		pcl::fromROSMsg(input,cloud);

		livox_obj_cloud.clear();
		for (size_t i = 0; i < cloud.size(); i++)
		{
			if ((abs(cloud.points[i].x)<(livox_range_x/2))&&(abs(cloud.points[i].y)<(livox_range_y/2))&&(cloud.points[i].z<(livox_range_z-1)))
			{
				livox_obj_cloud.push_back(cloud.points[i]);
			}
			// if ((abs(cloud.points[i].x)<(livox_range_x/2))&&(abs(cloud.points[i].y)<(livox_range_y/2))&&(cloud.points[i].z<livox_range_z))
			// {
			// 	livox_obj_cloud.push_back(cloud.points[i]);
			// }
		...
		sensor_msgs::PointCloud2 output;
		pcl::toROSMsg(livox_obj_cloud,output);
		output.header.frame_id = "livox";
		livox_obj_pub.publish(output);


		pcl::toROSMsg(cloud,output);
		output.header.frame_id = "livox";
		livox_pub.publish(output);

		// Eigen::Matrix4f transform_1 = Eigen::Matrix4f::Identity();
		// Eigen::Matrix3f rotation_matrix1;
		// rotation_matrix1 =
		// Eigen::AngleAxisf(0, Eigen::Vector3f::UnitZ()) *
		// Eigen::AngleAxisf(-3.1415926535/2, Eigen::Vector3f::UnitY()) *
		// Eigen::AngleAxisf(-3.1415926535/2, Eigen::Vector3f::UnitX());
		// transform_1.block<3,3>(0,0) =rotation_matrix1;
...
protected:
	//创建节点 接受者 发布者
	ros::NodeHandle nh;
	ros::Subscriber pcl_sub,pcl_sub_livox;
	ros::Publisher pcl_pub,pass_pub,obj_cloud_pub,obj_cluster_cloud_pub,tof_org_pub,livox_pub,livox_obj_pub;
	...
	double range_x,range_y,range_z,livox_range_x,livox_range_y,livox_range_z,detec_y,nearest_dis,car_length,car_width,car_height;
	int nan_ground_num;
	double angle_the;
	...
	pcl::PointCloud<pcl::PointXYZ> tof_transformed_cloud;
	...
	std::vector<double> T_tof_to_livox_vel;
	Eigen::Matrix4f T_tof_to_livox_M; 

	ros::Time last_obj_time;
	ros::Time now_obj_time;
	bool face_obj;
	double obj_wait_time;
	int enable_planning;
	int planning_Num;
...
public:
	int obj_symble;
	float obj_distance;
	bool use_planning,do_A_star;
	
	pcl::PointCloud<pcl::PointXYZ> livox_obj_cloud;
	pcl::PointCloud<pcl::PointXYZ> tof_obj_cloud;
