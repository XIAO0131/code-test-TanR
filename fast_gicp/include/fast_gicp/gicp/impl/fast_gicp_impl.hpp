...
#pragma omp parallel for num_threads(num_threads_) reduction(+ : sum_errors) schedule(guided, 8)
  for (int i = 0; i < input_->size(); i++) {
    ...
    if (std::isnan(mahalanobis_[i](0,0))||std::isnan(mahalanobis_[i](1,1))||std::isnan(mahalanobis_[i](2,2))||std::isnan(mahalanobis_[i](3,3))||std::isnan(mahalanobis_[i](4,4))||std::isnan(mahalanobis_[i](5,5)))
    {
      continue;
    }
    if (std::isinf(mahalanobis_[i](0,0))||std::isinf(mahalanobis_[i](1,1))||std::isinf(mahalanobis_[i](2,2))||std::isinf(mahalanobis_[i](3,3))||std::isinf(mahalanobis_[i](4,4))||std::isinf(mahalanobis_[i](5,5)))
    {
      continue;
    }
...
#pragma omp parallel for num_threads(num_threads_) schedule(guided, 8)
  for (int i = 0; i < cloud->size(); i++) {
    if (std::isnan(cloud->at(i).x)||std::isnan(cloud->at(i).y)||std::isnan(cloud->at(i).z))
    {
      continue;
    }
    if (std::isinf(cloud->at(i).x)||std::isinf(cloud->at(i).y)||std::isinf(cloud->at(i).z))
    {
      continue;
    }
